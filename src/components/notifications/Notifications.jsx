        ...
        >
          <div className="px-4 py-3 border-b border-ink-500 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="w-[1px] h-3 bg-ink-500 mx-1"></div>
              <button
                onClick={() => setOpen(false)}
                className="text-zinc-400 hover:text-white h-6 w-6 flex items-center justify-center rounded-md hover:bg-ink-700"
              >
                <IconClose size={14} />
              </button>
            </div>
        ...
        >
          ...
          <div className="relative w-full max-w-[640px] max-h-[85vh] bg-ink-800 border border-ink-500 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            <div className="px-5 py-4 border-b border-ink-500 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2 overflow-hidden pe-2">
                <IconBell size={18} className="shrink-0" />
                <h3 className="text-[16px] font-bold truncate">{t("allNotifs")}</h3>
                {unread > 0 && (
                  <span className="label-peyda px-2 py-0.5 rounded-full bg-brand-red/20 text-brand-redSoft border border-brand-red/30 shrink-0 hidden sm:inline-flex">
                    {unread} {t("newBadge")}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <button
                  onClick={markAll}
                  className="text-[12px] text-zinc-400 hover:text-white shrink-0 hidden sm:block"
                >
                  {t("markAll")}
                </button>
                <div className="w-[1px] h-4 bg-ink-500 mx-1 hidden sm:block"></div>
                <button
                  onClick={() => setModal(false)}
                  className="h-8 w-8 rounded-lg flex flex-shrink-0 items-center justify-center text-zinc-400 hover:text-white hover:bg-ink-700 bg-ink-800 border border-ink-500 sm:border-transparent sm:bg-transparent"
                  aria-label="بستن"
                >
                  <IconClose size={16} />
                </button>
...
export { MOCK_NOTIFS_DEFAULT, NotifIcon, NotificationBell };
