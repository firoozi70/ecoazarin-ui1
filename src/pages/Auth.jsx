...
function AcceptTerms() {
  ...
  return (
    <p className="text-center text-[11.5px] text-zinc-500 mt-10 tab-anim-soft leading-relaxed font-medium">
      {before}
      <a href="#" className={linkCls}>{t('rules')}</a>
      {between}
      <a href="#" className={linkCls}>{t('privacy')}</a>
      {after}
    </p>
  );
}
    ...
    switch (view) {
              ...
              <div className="space-y-5 tab-anim">
                <div className="flex items-center gap-6 mb-2">
                  <label className="flex items-center gap-2.5 text-[13px] cursor-pointer group">
                    ...
                    <span className={method === 'password' ? 'text-white light:text-zinc-900 font-bold' : 'text-zinc-400 group-hover:text-zinc-300 transition-colors'}>{t('password')}</span>
                  </label>
                  <label className="flex items-center gap-2.5 text-[13px] cursor-pointer group">
                    <div className="relative flex items-center justify-center">
                    ...
                    <span className={method === 'otp' ? 'text-white light:text-zinc-900 font-bold' : 'text-zinc-400 group-hover:text-zinc-300 transition-colors'}>{t('otp')}</span>
                  </label>
                </div>
                ...
                <button
                  ...
                  {method === 'password' ? t('enterAccount') : t('sendOtp')}
                </button>
              </div>
            ) : (
              <div className="space-y-5 tab-anim">
      ...
      case 'forgot':
        return (
          <div className="tab-anim space-y-5">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-black mb-2">{t('forgotTitle')}</h2>
              <p className="text-[13px] text-zinc-400 leading-relaxed">{t('forgotDesc')}</p>
            </div>
            <div>
              <label className="block text-[12.5px] font-medium text-zinc-400 mb-2">{t('mobileNumber')}</label>
              <input type="tel" dir="ltr" value={mobile} onChange={(e) => setMobile(e.target.value)} placeholder="0912 345 6789" className="w-full h-12 bg-ink-900/60 light:bg-zinc-50 border border-ink-500 light:border-zinc-300 rounded-xl px-4 text-left font-mono text-[15px] text-white light:text-zinc-900 outline-none focus:border-brand-red/60 focus:bg-ink-900 light:focus:bg-white focus:shadow-[0_0_0_4px_rgba(230,57,70,0.1)] transition-all placeholder-stripe placeholder:opacity-50" />
      ...
      case 'otp':
        return (
          <div className="tab-anim space-y-6 text-center py-2">
            <button onClick={() => setView(context === 'register' ? 'register' : context === 'forgot' ? 'forgot' : 'login')} className="absolute top-5 end-5 w-8 h-8 flex items-center justify-center rounded-full bg-ink-800/50 light:bg-zinc-100 text-zinc-400 hover:text-white light:hover:text-zinc-800 transition-colors" aria-label={t('close')}>
            ...
            <div>
              ...
              <h2 className="text-xl md:text-2xl font-black mb-2">{t('verifyMobile')}</h2>
              <p className="text-[13px] text-zinc-400 leading-relaxed max-w-[280px] mx-auto">
                {t('verifyDesc', { mobile: mobile || '…' })}
              </p>
            </div>
      ...
      case 'profile':
        return (
          <div className="tab-anim space-y-5">
            <div>
              <h2 className="text-xl md:text-2xl font-black mb-2 text-brand-greenSoft">{t('completeProfile')}</h2>
              <p className="text-[13px] text-zinc-400 leading-relaxed">{t('profileDesc')}</p>
            </div>
            <div className="pt-2">
              <label className="block text-[12.5px] font-medium text-zinc-400 mb-2">{t('fullName')}</label>
              <input type="text" placeholder={t('fullNamePlaceholder')} className="w-full h-12 bg-ink-900/60 light:bg-zinc-50 border border-ink-500 light:border-zinc-300 rounded-xl px-4 text-white light:text-zinc-900 outline-none focus:border-brand-green/60 focus:bg-ink-900 light:focus:bg-white focus:shadow-[0_0_0_4px_rgba(16,185,129,0.1)] transition-all placeholder-stripe placeholder:opacity-50" />
            </div>
            <div>
      ...
      case 'reset':
        return (
          <div className="tab-anim space-y-5">
            <div>
              <h2 className="text-xl md:text-2xl font-black mb-2">{t('resetTitle')}</h2>
              <p className="text-[13px] text-zinc-400 leading-relaxed">{t('resetDesc')}</p>
            </div>
            <div className="pt-2">
          ...
          <div className="relative w-full max-w-[500px] aspect-square flex items-center justify-center">
            ...
            <div className="absolute top-[10%] end-[-10%] bg-ink-800/90 light:bg-white backdrop-blur-xl border border-ink-500 light:border-zinc-200 rounded-2xl p-4 shadow-2xl animate-[fadeinUp_1s_ease-out_both] z-20" style={{ animationDelay: '100ms' }}>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="text-[14px] font-black">{t('smartAlert')}</div>
              </div>
              <div className="text-[11.5px] text-zinc-400 mt-1 max-w-[140px] leading-relaxed">{t('smartAlertDesc')}</div>
            </div>
            ...
            <div className="absolute top-[40%] start-[-15%] bg-ink-800/90 light:bg-white backdrop-blur-xl border border-ink-500 light:border-zinc-200 rounded-2xl p-4 shadow-2xl animate-[fadeinUp_1s_ease-out_both] z-20" style={{ animationDelay: '300ms' }}>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="text-[14px] font-black">{t('specialistEdu')}</div>
              </div>
              <div className="text-[11.5px] text-zinc-400 mt-1 max-w-[140px] leading-relaxed">{t('specialistEduDesc')}</div>
            </div>
            ...
            <div className="absolute bottom-[5%] start-[10%] bg-ink-800/90 light:bg-white backdrop-blur-xl border border-ink-500 light:border-zinc-200 rounded-2xl p-4 shadow-2xl animate-[fadeinUp_1s_ease-out_both] z-20" style={{ animationDelay: '500ms' }}>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[15px] font-black">{t('technicalAnalysis')}</span>
                <span className="text-[10px] font-bold text-brand-green flex items-center ltr-num" dir="ltr">{t('successRate')}</span>
              </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col relative z-50">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-6 md:p-8">
          <div className="w-full max-w-[400px] relative">
            <div className="mb-8">
              <h1 className="text-2xl md:text-[28px] font-black mb-3 text-white light:text-zinc-900 tracking-tight">{t('authWelcome')}</h1>
              <p className="text-[13.5px] text-zinc-400 leading-relaxed font-medium">{t('authDesc')}</p>
            </div>
...
export { isEn, grad, AcceptTerms, AuthApp };
