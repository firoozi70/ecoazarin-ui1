            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  <span className={`text-[12px] md:text-[13px] tracking-tight ${active ? 'font-extrabold text-black' : 'font-semibold'}`}>{window.t ? window.t('tab' + tab.id.charAt(0).toUpperCase() + tab.id.slice(1)) : tab.label}</span>
                </button>);
            })}
          </div>
        </div>
      ...
      <div
        ...
        <div className="bg-ink-800/95 border border-ink-500 backdrop-blur-xl rounded-2xl p-2 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] w-[320px] max-w-[90vw]">
          <div className="grid grid-cols-2 gap-1">
            {tabs.map((t) => {
              const Icon = TAB_ICON[t.icon];
              const active = t.id === activeId;
              return (
                <button
                  key={t.id}
                  onClick={() => {onChange(t.id);setOpen(false);window.scrollTo({ top: 0, behavior: 'smooth' });}}
